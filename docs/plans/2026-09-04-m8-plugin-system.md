...
四类插件走**同一套契约**（plugin.json 清单 + JSON 行 IPC + 宿主代存配置），区别只是清单声明：
渲染类声明窗口/穿透属性；壁纸源与规则类 `hwnd=0` 跳过挂载。未来新类别（桌面自动化、盯盘
小组件等）不落此表也可直接接入，架构不动。

### 2.1 内容渲染类（有窗口，挂壁纸之上、图标之下）

| 插件 | 说明 |
|---|---|
| **桌面宠物**（首发官方插件） | 角色在桌面走动、攀爬屏幕边缘、被拖动、点击互动 |
| **桌面小组件** | 时钟、天气、日历、CPU/GPU/内存/网速监控仪表（Rainmeter 经典生态位） |
| **便签/待办** | 桌面速记贴纸 |
| **氛围特效** | 雨滴/雪花/樱花飘落——叠加壁纸上、点击穿透 |
| **音频可视化** | 随系统声音律动的频谱/波形 |

### 2.2 壁纸源类（无窗口或极轻，向宿主供图）

- Bing 每日一图（自动换每日壁纸）
- Unsplash / Pexels 随机高质量图（按关键词主题）
- 本地文件夹定时轮换
- 时间主题（白天亮图 / 夜晚暗图）

### 2.3 自动化规则类（无窗口，后台听事件）

- **自动归盒**：新文件按类型/日期自动进指定收纳盒（如"新装软件快捷方式 → 软件 盒"）
- **桌面健康**：图标超量提醒整理、临时文件超期清理提醒
- **番茄钟/喝水提醒**：到点宠物跑出来提醒（与宠物插件联动）

### 2.4 图标增强类（远期预留，涉及图标层内部扩展）

- 图标角标/状态标记（网盘同步中、下载进度）
- 文件夹缩略图定制
...
- **渲染器抽象（IPetRenderer）**：按素材格式自动选择——
  - `*.model3.json` → **Live2D 渲染器**（WebView2 透明窗口 + pixi.js + pixi-live2d-display；
    眼神跟随/物理摆动/部位命中/表情渐变；内存 ~100-150MB、GPU 1-3%）
  - `frames/*.png` → **Sprite 渲染器**（帧序列 + 状态机；零依赖兜底 10-30MB；
    低配/无 WebView2 降级路径，也是 Live2D 许可风险的后手）
  - 行为层（走/爬/拖/互动）两渲染器完全共用，仅角色表现不同
- **行为状态机**：idle（原地小动作）→ walk（沿屏底行走）→ climb（贴屏幕左右边缘攀爬）→
  fall（从顶部落下，简单重力）→ drag（被鼠标拖动，跟随+摆动）→ interact（点击反应：表情/动作）
- **交互**（见 5.2）
- **多屏**：可走到相邻屏（MonitorsInfo 的虚拟桌面坐标系）；默认只在主屏，配置项开启多屏
- **全屏礼让**：收到 Pause → 走到屏幕底边趴下/淡出；Resume → 复活（宠物自己演"睡觉"）
- **许可注**：Cubism SDK 免费档（个人/年营收 ≤1000 万日元）适用当前免费上架场景；
  默认模型用官方免费示例素材（遵守署名条款），用户自换模型版权自负
...
|---|---|
| 点击 | 表情/动作反应（惊讶、开心跳一下）|
| 拖拽 | 跟随移动 + 惯性摆动，扔出去坠落弹跳 |
| 双击 | 特殊动作（转圈、打滚）|
| 摸/戳不同部位 | 不同反应（摸头开心、戳肚子生气）|
| 长时间不理 | 自己找乐子：睡觉、发呆、走开 |
| 拖到屏幕边缘 | 攀爬/倒挂 |

**Live2D 独有**：眼神跟随（鼠标在屏幕任何位置、即使悬在其他窗口上，眼睛和头都看向鼠标——
宿主全局鼠标位置广播）、模型 HitAreas 部位命中（点哪是哪带渐变）、物理摆动（头发/尾巴随
拖动甩动）、表情混合渐变。

**实现要点**：
- 不规则命中 = per-pixel alpha 透明窗口（点在角色透明像素穿透到桌面，WPF 天然行为）
- 拖拽 vs 点击 = 位移阈值判定（复用图标拖拽的 arm/threshold 机制）
- 全局鼠标位置 = 宿主低级鼠标钩子（与键盘查找的钩子封装同族），广播给宠物
- WebView2 内鼠标事件 → pixi 命中模型部位 → 触发 motion，同时冒泡 WPF 层处理拖拽

**T3+ 远期（不进首发）**：喂食/好感度、点击说话气泡、番茄钟插件联动宠物提醒、快捷键召唤。
