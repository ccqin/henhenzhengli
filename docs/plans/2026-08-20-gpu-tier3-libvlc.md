...
- **切换壁纸坑（0c653d1）**：换壁纸时旧播放器 vout 尚在后台拆除，新播放器复用同一 HWND
  与残骸争抢 → libvlc 弃嵌入**自建顶层弹窗**（真机症状：切壁纸后桌面浮出两个 VLC 窗口）。
  修复 = 每次 Play 销毁重建宿主子 HWND（连带拆旧 vout 窗口树）+ 创建后立即铺满父客户区。
  另修：StartVideoReport 的 Tick 订阅每次切换累积 → 位置上报重复 → 主进程组同步误判漂移
