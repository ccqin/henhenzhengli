...
**Goal:** 图标层从「单窗口铺主屏」升级为「每显示器一个 IconLayerWindow」：Fence/散落图标按**显示器持久 ID** 归属，布局每屏独立持久化；拔线/插回/换顺序/改分辨率/DPI 变化后布局不串屏、不丢失。真机环境：双显示器（已确认）。
...
| M3-T7 | 接线收尾 + 冒烟 + tag `m3-multimon` | 集成 | 双屏全流程冒烟 | T1–T6 |
...
└── App.xaml.cs                 # 改：单 _iconLayer → host；sync.Changed → host.Dispatch(diff)
...
- [ ] 双屏冒烟清单：
  ...
  4. 重启 explorer → 接管保持（双屏）
  5. 双击空白隐藏（每屏独立生效）
  6. 托盘退出 → 原生图标恢复、config 完整
...
- 与路线图 M3.1/M3.2/M3.3 对齐：T1=M3.1（QueryDisplayConfig），T3/T4/T5=M3.2（每屏窗口 + 归属，加了跨屏拖拽增量），T6=M3.3（热插拔/DPI）。
...
- 测试边界：Core（MonitorAssignment + config 兼容）TDD；Native 持久 ID 与 App 多窗口行为 = 双屏真机验收（无法可靠自动化，遵循项目既定测试策略）。
